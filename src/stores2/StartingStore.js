...
class StartingStore {
  ...
  getArtistFollowArtwork = () => {
    ...
    this.api.getArtistFollowArtwork(userData.accEmailAddress).then((resp) => {
      console.log(resp.data, 'data');
    ...
    return new Promise((resolve, reject) => {
      this.api.getartworkInfo().then((resp) => {
        ...
        if (userData === null || userData === false) {
          let listofUserCategoriesArtwork = resp.data.map((art) => {
            this.listofFilteredUserArtworkCategories.push(art);
            return art;
          });
        } else {
          let listofUserCategoriesArtwork = resp.data.filter((art) => {
  ...
  editToCart = (data) => {
    console.log('data');
    let currData = toJS(data[0]);
    let userData = JSON.parse(sessionStorage.getItem('userData'));
    console.log(currData, 'qwer');
